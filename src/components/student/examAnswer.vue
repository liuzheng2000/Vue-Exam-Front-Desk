    ...
    <div id="answer">
      ...
      <div class="top">
        <ul class="item">
          ...
          <li><video ref="video" width="160" height="80" autoplay></video></li>
          <li @click="flag = !flag">
          ...
          <div class="right">
            ...
            <div class="operation">
              <ul class="end">
                ...
                <li id="testButt" @click="RandomPhotography()">
...
export default {
  ...
  beforeDestroy() {
    ...
    this.destroy();
    this.callCameraFalse();
    this.clearVueInterval();
  },

  methods: {
    clearVueInterval(){
       clearInterval(this.VueInterval);
      this.VueInterval = null // 这里最好清除一下，回归默认值
      // 众所周知，定时器返回一个随机整数，用于表示定时器的编号，后面通过名字可以取消这个定时器的执行。
      console.log(this.VueInterval);
    ...
    RandomPhotography() {
      ...
      this.uploadPhotography(this.headImgSrc);
    },

    //上传拍摄的图片
    uploadPhotography(headImgSrc) {
      this.$axios({
        url: `/examApi/exam/getPhoto`,
        method: "post",
        data: {
    ...
    callCamera() {
      // H5调用电脑摄像头API
      this.gumStream = navigator.mediaDevices
        .getUserMedia({
          video: true,
        })
        .then((success) => {
        ...
        .catch((error) => {
          this.$alert("摄像头开启失败,请更换浏览器", "考试注意", {
    ...
    SizeChange() {
      ...
      window.onresize = () => {
        if (screenfull.isFullscreen) {
          ...
          this.uploadCount(this.count);
          this.$alert(
            "请不要退出全屏,信息上报,多次上报后进行自动提交",
            "考试注意",
            {
              confirmButtonText: "确定",
              callback: (action) => {
